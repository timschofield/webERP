...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedAccount) AND $InputError !=1) {
		...
		$sql = "UPDATE chartmasterPT SET accountname='" . $_POST['AccountName'] . "',
						group_='" . $_POST['Group'] . "'
	...
	/*SelectedAccount is null cos no item selected on first time round so must be adding a	record must be submitting new entries */
		...
		$sql = "INSERT INTO chartmasterPT (accountcode,
						accountname,
						group_)
...
//the link to delete a selected record was clicked instead of the submit button
	$sql="DELETE FROM chartmasterPT WHERE accountcode= '" . $SelectedAccount ."'";
	$result = DB_query($sql);
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedAccount)) {
		...
		$sql = "SELECT accountcode, accountname, group_ FROM chartmasterPT WHERE accountcode='" . $SelectedAccount ."'";
...
then none of the above are true and the list of chartmasterPT will be displayed with
...
or deletion of the records*/
	...
	$sql = "SELECT accountcode,
		...
		FROM chartmasterPT,
			accountgroups
		WHERE chartmasterPT.group_=accountgroups.groupname
		ORDER BY chartmasterPT.accountcode";

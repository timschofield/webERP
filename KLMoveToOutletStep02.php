...
$Title = _('KL Move To Outlet -> Step 02');
	...
	if (DB_num_rows($result) != 0){
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ((($myrow['qohkantor'] + $myrow['qohotherlocs']) == $myrow['qohtotal'])
				...
				$NewDiscountCategory = '<a href="' . $RootPath . '/KLChangeToOutlet.php?Item=' . $myrow['stockid'] . '&Discount='. $myrow['discountcategory'] . '&Action=Change">' . $myrow['discountcategory'] . '</a>';
			}else{
		...
		prnMsg("No items to be moved to outlet at the moment", "success");
	}

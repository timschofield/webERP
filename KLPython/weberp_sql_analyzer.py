...
def analyze_weberp_sql_for_strict_mode(repo_path):
    """
    ...
    potential_issues = {
        ...
        'non_full_group_by_columns': [] # Note: This is more about behavior than error for your specific new SQL_MODE
    }

    # Regex for common SQL query patterns. This is a simplification and not exhaustive.
    # It looks for basic INSERT/UPDATE/SELECT structures.
    # A more robust solution would require a full SQL parser.
    ...
    select_division_pattern = re.compile(r'SELECT\s+.*\/\s*0.*', re.IGNORECASE) # division by literal 0
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for query in queries:
                    ...
                    # This is hard to detect perfectly without knowing schema and actual data,
                    # but we can flag common risky patterns or any INSERT/UPDATE as needing attention.
                    if insert_update_pattern.search(query):
                        # This is a general flag, as actual truncation depends on data vs column definition
 ...
 #                       )
                        ...
                        if re.search(r"\'0000-00-00(\s00:00:00)?\'", query):
                    ...
                    if select_division_pattern.search(query):
                    ...
                    # Since your target SQL_MODE *does not* include ONLY_FULL_GROUP_BY, this is less likely to cause an error,
                    # but more about potential non-deterministic results if the original dev relied on it.
                    ...
                    if match_group_by:
                        ...
                        # This is a very basic check. Real analysis needs to distinguish aggregate functions.
                        # We flag if any selected column is not in GROUP BY and not an aggregate function.
                        # This is heuristic and prone to false positives/negatives without full SQL parsing.
                        for col in selected_columns:
                            # Simple check for common aggregate functions
                            if not any(agg_func in col.upper() for agg_func in ['SUM(', 'COUNT(', 'AVG(', 'MAX(', 'MIN(']) \
                               and col not in group_by_columns:
                                potential_issues['non_full_group_by_columns'].append(
                                    f"File: {os.path.basename(file_path)}\n  Query (potential non-deterministic GROUP BY results without ONLY_FULL_GROUP_BY, though your new mode allows it):\n  {query[:200]}..."
                                )
...
#        for issue in set(potential_issues['strict_trans_tables_truncation']): # Using set to show unique queries/files
#            print(f"- {issue}\n")
...
#        print("\nNo explicit problematic queries found for STRICT_TRANS_TABLES (truncation/conversion), but extensive testing is still advised.")
    ...
    if potential_issues['strict_trans_tables_invalid_date']:
        ...
        for issue in set(potential_issues['strict_trans_tables_invalid_date']):
            print(f"- {issue}\n")
        print("Action: Modify application logic to use NULL or a valid default date (e.g., '1970-01-01') instead of '0000-00-00'.")
    else:
        print("No explicit '0000-00-00' date insertions found, but check for other invalid date patterns.")
    ...
    if potential_issues['error_for_division_by_zero']:
        ...
        for issue in set(potential_issues['error_for_division_by_zero']):
            print(f"- {issue}\n")
    ...
    if potential_issues['non_full_group_by_columns']:
        ...
        for issue in set(potential_issues['non_full_group_by_columns']):
            print(f"- {issue}\n")
    ...
    print("1.  **PHP Version Compatibility:** Ensure your webERP's PHP code is compatible with the PHP version on AlmaLinux 8.0 (likely PHP 7.2 or higher). This often involves dealing with deprecated `mysql_*` functions (if any exist, webERP typically uses `mysqli`) and other language changes.")
    print("2.  **Database Collation:** Verify that your database and table collations are appropriate for UTF-8 (e.g., `utf8mb4_unicode_ci` or `utf8mb4_general_ci`) if you store multi-byte characters. Changes in default collation can lead to sorting or comparison issues.")
    print("3.  **MySQLa_DB_Query() and DB_query()**: webERP encapsulates database interactions. Look closely at how these functions are used, especially with complex WHERE clauses or data manipulations.")
    print("4.  **Full Regression Testing:** The most critical step is to perform extensive testing on a non-production environment after migration. This script is a preliminary check, not a guarantee.")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(repo_path):
        ...
        analyze_weberp_sql_for_strict_mode(repo_path)

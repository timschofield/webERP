...
if ($ProcessSection01){
	...
	***************************************************************************************/
	...
	if ($KL_SystemAdmin 
		...
		$StartTime = microtime(true);
		ItemsWithoutRetailPrice("SETKLA", $_SESSION['Price_Factor_Minimum_KL'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("TESTKA", $_SESSION['Price_Factor_Minimum_KL'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("STABKA", $_SESSION['Price_Factor_Minimum_KL'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("NOPOKA", $_SESSION['Price_Factor_Minimum_KL'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		ItemsWithoutRetailPrice("SETBLA", $_SESSION['Price_Factor_Minimum_Blink'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("TESTBA", $_SESSION['Price_Factor_Minimum_Blink'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("STABBA", $_SESSION['Price_Factor_Minimum_Blink'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("NOPOBA", $_SESSION['Price_Factor_Minimum_Blink'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		ItemsWithoutRetailPrice("SETGEA", $_SESSION['Price_Factor_Minimum_General'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("TESTGA", $_SESSION['Price_Factor_Minimum_General'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("STABGA", $_SESSION['Price_Factor_Minimum_General'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsWithoutRetailPrice("NOPOGA", $_SESSION['Price_Factor_Minimum_General'], $RootPath);
		TimeNeededForExecution("ItemsWithoutRetailPrice", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
	}
	...
	if ($KL_SystemAdmin 
		OR $KL_SalesTeamManager){
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("SETKLA", $_SESSION['Price_Factor_Minimum_KL'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("TESTKA", $_SESSION['Price_Factor_Minimum_KL'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("STABKA", $_SESSION['Price_Factor_Minimum_KL'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("NOPOKA", $_SESSION['Price_Factor_Minimum_KL'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("SETBLA", $_SESSION['Price_Factor_Minimum_Blink'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("TESTBA", $_SESSION['Price_Factor_Minimum_Blink'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("STABBA", $_SESSION['Price_Factor_Minimum_Blink'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("NOPOBA", $_SESSION['Price_Factor_Minimum_Blink'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("SETGEA", $_SESSION['Price_Factor_Minimum_General'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("TESTGA", $_SESSION['Price_Factor_Minimum_General'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("STABGA", $_SESSION['Price_Factor_Minimum_General'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += PriceBelowStandard("NOPOGA", $_SESSION['Price_Factor_Minimum_General'], 0, $RootPath);
		TimeNeededForExecution("PriceBelowStandard", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
	}
}
...
if ($ProcessSection02){
	...
	if ($KL_SystemAdmin 
		OR $KL_SalesTeamManager){
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("TESTKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Minimum_TopSales_KL'], 0, 50, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("STABKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Minimum_TopSales_KL'], 0, 50, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("NOPOKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Minimum_TopSales_KL'], 0, 50, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("TESTBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Minimum_TopSales_Blink'], 0, 40, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("STABBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Minimum_TopSales_Blink'], 0, 40, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooCheap("NOPOBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Minimum_TopSales_Blink'], 0, 40, 60, $RootPath);
		TimeNeededForExecution("ItemsTooCheap", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("TESTKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Maximum_BottomSales_KL'], 0, 500, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("STABKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Maximum_BottomSales_KL'], 0, 500, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("NOPOKA", $_SESSION['Price_Factor_Minimum_KL'], $_SESSION['Price_Factor_Maximum_BottomSales_KL'], 0, 500, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;

		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("TESTBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Maximum_BottomSales_Blink'], 0, 300, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("STABBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Maximum_BottomSales_Blink'], 0, 300, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
		$StartTime = microtime(true);
		$IssuesFound += ItemsTooExpensive("NOPOBA", $_SESSION['Price_Factor_Minimum_Blink'], $_SESSION['Price_Factor_Maximum_BottomSales_Blink'], 0, 300, 60, $RootPath);
		TimeNeededForExecution("ItemsTooExpensive", $StartTime, $KL_SystemAdmin);
		$NumberOfTestExecuted++;
	}

...
if (isset($_POST['ProcessInvoice']) and $_POST['ProcessInvoice'] != '') {
	...
	foreach ($_SESSION['Items' . $identifier]->LineItems as $OrderLine) {
		...
		if ($OrderLine->QtyDispatched != 0 and $OrderLine->QtyDispatched != '' and $OrderLine->QtyDispatched) {
			...
			if ($_SESSION['CompanyRecord']['gllink_stock'] == 1 and $OrderLine->StandardCost != 0 and !$IsAsset) {
				...
				// KL RICARD 
				// Begin of Consignment sales for PTADU MODIFICATIONS. 
				...
				if (!ItemInList($Area, LIST_SALES_AREAS_PTADU)){  
					...
					if (DB_num_rows($Result)==0) {
						prnMsg(__('ERROR-INV-00001. The location used as warehouse delivery can not sell PTADU items.'),'error');
						include('includes/footer.php');
						exit;
					}
					...
					if ($MyRow['partnercode']=='NORETAIL'){
						prnMsg(__('ERROR-INV-00002. The location used as warehouse delivery can not sell PTADU items.'),'error');
						include('includes/footer.php');
						exit;
					}
			...
			if (ItemInList($CategoryID, LIST_STOCK_CATEGORIES_SHOP_PACKAGING)){
				...
				$SQL = "INSERT INTO packagingused (
						...
						VALUES ('" .$_SESSION['ProcessingOrder'] . "',
							...
							'" . $OrderLine->QtyDispatched . "',
							CURRENT_DATE)";
				$ErrMsg = __('ERROR: Contact the office!!!  -> AdjustPackagingMovement-1030');
				$Result = DB_query($SQL,$ErrMsg,'',true);
			// KL RICARD END
			}
	...
	if ($_SESSION['InvoicePortraitFormat'] == 0) {
		...
		echo '<img src="' . $RootPath . '/css/' . $Theme . '/images/printer.png" title="' . __('Print') . '" alt="" />' . ' ' . '<a target="_blank" href="' . $RootPath . '/PrintCustTransPortrait.php?FromTransNo=' . $InvoiceNo . '&amp;InvOrCredit=Invoice&amp;PrintPDF=True&orientation=portrait">' . __('Print this invoice') . ' (' . __('Portrait') . ')</a><br /><br />';
	}
	...
	echo '<field>
			<label for="Consignment">', __('Tracking Number'), ':</label>
			<input type="text" maxlength="20" size="20" name="Consignment" value="', $_POST['Consignment'], '" />
			<fieldhelp>', __('Enter the tracking number from shipper to enable tracking of the delivery in the event of customer proof of delivery issues.'), '</fieldhelp>
		</field>';

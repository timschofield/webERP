...
if ($_SESSION['PO'.$identifier]->Status != 'Printed') {
	prnMsg( __('Purchase orders must have a status of Printed before they can be received').'.<br />' .
		__('Order number') . ' ' . $_GET['PONumber'] . ' ' . __('has a status of') . ' ' . _($_SESSION['PO'.$identifier]->Status), 'warn');
	include('includes/footer.php');
	exit();
}
		...
		if ($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode != $MyRow['glcode'] OR
			...
			if ($Debug==1) {
				echo '<table class="selection">
					...
					<tr>
						<td>' . __('Stock Code of the Line Item') . ':</td>
						<td>' .  $_SESSION['PO'.$identifier]->LineItems[$LineNo]->StockID . '</td>
						<td>' . $MyRow['itemcode'] . '</td>
					</tr>
					<tr>
						<td>' . __('Order Quantity of the Line Item') . ':</td>
					...
					<tr>
						<td>' . __('Quantity of the Line Item Already Received') . ':</td>

...
/*First off do the stock check file stuff */
	if ($_POST['MakeStkChkData']=='New'){
		...
		if (DB_error_no() !=0) {
			...
			prnMsg(__('The inventory quantities could not be added to the freeze file because') . ' ' . DB_error_msg(),'error');
	...
	if ($_POST['MakeStkChkData']=='AddUpdate'){
		...
		if (DB_error_no() !=0) {
			...
			include('includes/header.php');
			prnMsg(__('The old quantities could not be deleted from the freeze file because') . ' ' . DB_error_msg(),'error');
			echo '<br /><a href="' . $RootPath . '/index.php">' . __('Back to the menu') . '</a>';
			if ($Debug==1){
		  			echo '<br />' . $SQL;
			}
		...
		$Result = DB_query($SQL,'','',false,false);
		if (DB_error_no() !=0) {
			$Title = __('Stock Freeze - Problem Report');
			include('includes/header.php');
			prnMsg(__('The inventory quantities could not be added to the freeze file because') . ' ' . DB_error_msg(),'error');
			echo '<br /><a href="' . $RootPath . '/index.php">' . __('Back to the menu') . '</a>';
			if ($Debug==1){
	...
	if (DB_error_no() !=0) {
		$Title = __('Stock Sheets') . ' - ' . __('Problem Report') . '.... ';
		include('includes/header.php');

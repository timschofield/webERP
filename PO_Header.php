...
if ((isset($_POST['UpdateStatus']) and $_POST['UpdateStatus'] != '')) {
	...
	// none of the order has been received
				if ($AuthorityLevel > $OrderTotal) {
...
if ((isset($_POST['UpdateKLStatus']) AND $_POST['UpdateKLStatus'] != '')) {
	...
	if ($_SESSION['ExistingOrder'] == 0) {
		prnMsg(__('This is a new order. It must be created before you can change the KL status'), 'warn');
	...
	if ($OKToUpdateKLStatus == 1) {
		...
		$_SESSION['PO'.$identifier]->KLAgreedDeliveryDate = $_POST['KLAgreedDeliveryDate'];
		...
		$_SESSION['PO'.$identifier]->KLCustomsDate = $_POST['KLCustomsDate'];
		$_SESSION['PO'.$identifier]->KLArrivalDate = $_POST['KLArrivalDate'];
		$SQL = "UPDATE purchorders 
				SET klstatus='" . $_POST['KLStatus'] . "',
					agreeddeliverydate='" . FormatDateForSQL($_POST['KLAgreedDeliveryDate']) . "',
					...
					customsdate='" . FormatDateForSQL($_POST['KLCustomsDate']) . "',
					arrivaldate='" . FormatDateForSQL($_POST['KLArrivalDate']) . "'
				WHERE purchorders.orderno ='" . $_SESSION['ExistingOrder'] . "'";

		$ErrMsg = __('The order KL status could not be updated because');
		$UpdateResult = DB_query($SQL, $ErrMsg);
...
if ($_SESSION['RequireSupplierSelection'] == 1 or !isset($_SESSION['PO' . $identifier]->SupplierID) or $_SESSION['PO' . $identifier]->SupplierID == '') {
	...
	if (!isset($_POST['LookupDeliveryAddress']) and (!isset($_POST['StkLocation']) or $_POST['StkLocation']) and (isset($_SESSION['PO' . $identifier]->Location) and $_SESSION['PO' . $identifier]->Location != '')) {
		...
		$_POST['KLShipmentDate'] = $_SESSION['PO' . $identifier]->KLShipmentDate;
		$_POST['KLShipmentAWB'] = $_SESSION['PO' . $identifier]->KLShipmentAWB;
		$_POST['KLCustomsDate'] = $_SESSION['PO' . $identifier]->KLCustomsDate;
		$_POST['KLArrivalDate'] = $_SESSION['PO' . $identifier]->KLArrivalDate;
...
KL RICARD COMMENTED OUT END */
	...
	if ($_SESSION['PO' . $identifier]->Status == '') { //then its a new order
		echo '<input type="hidden" name="KLStatus" value="1000" />' . __('Negotiating with supplier');
	} else {
/*		if (!isset($_POST['KLAgreedDeliveryDate'])){
...
*/		if (!isset($_POST['KLShipmentAWB'])){
		...
		$Result=DB_query($SQL);
		echo '<field>
				<label for="KLStatus">', __('KL Status'), ':</label>
		...
		echo '<field>
				<label for="KLAgreedDeliveryDate">', __('Agreed Delivery Date'), ':</label>

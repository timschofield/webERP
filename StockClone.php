...
//if ($_POST['New'] == 1) {
	...
	if (!isset($_POST['NextSerialNo'])) {
  	...
  	// KL RICARD
	echo '<field>
			<label for="lastcategoryupdate">' . __('Date last category') . ':</label>
			<input ' . (in_array('lastcategoryupdate',$Errors) ?  'class="inputerror"' : '' ) .'   type="text" class="number" name="lastcategoryupdate" size="12" maxlength="10" value="' . $_POST['lastcategoryupdate'] . '" />
		</field>';
	...
	echo '<field>
		...
		echo '<field>
				<input type = "hidden" name = "NextSerialNo" value = "0" />
			</field>';
	}
	...
	echo '<input type = "submit" name = "submit" value = "' . __('Clone Item') . '" />';

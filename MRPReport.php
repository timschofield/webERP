...
if (isset($_POST['PrintPDF']) AND $_POST['Part']!='') {
	...
	if (DB_error_no() !=0) {
		$Errors = 1;
		$Title = __('Print MRP Report Error');
		include('includes/header.php');
		prnMsg(__('The MRP calculation must be run before this report will have any output. MRP requires set up of many parameters, including, EOQ, lead times, minimums, bills of materials, demand types, master schedule etc'),'error');
	...
	if (DB_num_rows($Result) == 0) {
		...
		include('includes/header.php');
		prnMsg(__('The MRP calculation must be run before this report will have any output. MRP requires set up of many parameters, including, EOQ, lead times, minimums, bills of materials, demand types, master schedule, etc'), 'warn');
		echo '<br /><a href="' . $RootPath . '/index.php">' . __('Back to the menu') . '</a>';
		include('includes/footer.php');
		exit();
	}
	...
	if (isset($Errors)) {
		...
		include('includes/header.php');
		prnMsg( __('The MRP Report could not be retrieved by the SQL because') . ' '  . DB_error_msg(),'error');
		echo '<br /><a href="' .$RootPath .'/index.php">' . __('Back to the menu') . '</a>';
		if ($Debug==1){
			echo '<br />' . $SQL;
		}

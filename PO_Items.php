  ...
  * there will be an sql error  in SupplierInvoice.php without a valid GL Code
	*/
	if ($_SESSION['PO'.$identifier]->GLLink==1 OR $_SESSION['CompanyRecord']['gllink_creditors']==1){
		...
		$ErrMsg =  __('The account details for') . ' ' . $_POST['GLCode'] . ' ' . __('could not be retrieved because');
		$GLValidResult = DB_query($SQL, $ErrMsg,'',false,false);
		if (DB_error_no() !=0) {
			$AllowUpdate = false;
			prnMsg( __('The validation process for the GL Code entered could not be executed because') . ' ' . DB_error_msg(), 'error');
			if ($Debug==1){
				prnMsg (__('The SQL used to validate the code entered was') . ' ' . $SQL,'error');
			}
			include('includes/footer.php');
			exit();
		}

...
function DailyReorderLevelAdjustments($ShowMessages, $updateDB, $RootPath, $db){
...
*/
	...
	AdjustPackaging(30, 10, $ShowMessages, $updateDB, $RootPath, $db);
	...
	*/
	...
	if (DB_num_rows($result) != 0){
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ($myrow['categoryid'] == "BKCLOT"){
				$newRL = $myrow['reorderlevel'];
				$notes = "BKCLOT - Clothing. RL Not changed";
			}
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ($locationoverstock == ""){
				...
				if ($locationworst == ""){
					...
					if(($myrow['categoryid'] == 'ZYDISC') OR ($myrow['categoryid'] == 'ZYOUTL')){
...
function WorstLocationForItem($stockid, $stockcat, $kind, $maxdays, $db){
	...
	// if category is discount or outlet, then use priority for these categories
	if(($stockcat == 'ZYDISC') OR ($stockcat == 'ZYOUTL')){
...
function SetRLForLowSalesHighRL($maxdays, $oldRL, $maxRL, $minavailablestock, $ShowMessages, $updateDB, $RootPath, $db){
	...
	*/
	...
	$SQL = "SELECT 	stockmaster.stockid,
			...
			FROM 	stockmaster,locstock,locations
			WHERE 	stockmaster.stockid = locstock.stockid
					AND stockmaster.categoryid NOT IN ('SHDISP', 'ZYDISC', 'ZYOUTL', 'BKCLOT')
					AND (locstock.loccode = locations.loccode)
					AND (locstock.loccode LIKE 'TOK%')
					AND (locstock.quantity > 0)
...
function AdjustPackaging($DaysSales, $DaysRL, $ShowMessages, $updateDB, $RootPath, $db){
	...
	while ($iShop < $CountShops -1 ){ // take out the lst PACKA location
		...
		while ($iItem < $CountItem){
			AdjustPackagingItemByShop($Items[$iItem], $Shops[$iShop], $DaysSales, $DaysRL, $ShowMessages, $updateDB, $RootPath, $db);
			$iItem++;
		}
		$iShop++;
	}
...
function AdjustPackagingItemByShop($Item, $Shop, $DaysSales, $LongDaysSales, $DaysRL, $ShowMessages, $updateDB, $RootPath, $db){
	...
	if ($FieldName != 'ERROR'){
		...
		if (DB_num_rows($result) != 0){
			...
			$ShortRL = round($myrow['sales'] / $DaysSales * $DaysRL,0);
			$LongRL  = round($myrow['longsales'] / $LongDaysSales * $DaysRL,0);

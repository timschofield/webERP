...
if (isset($_POST['PrintPDF']) or isset($_POST['Spreadsheet']) or isset($_POST['View'])){
	...
	$SQL = "SELECT securitytokens.tokenid,
			...
			FROM securitytokens
			INNER JOIN securitygroups ON securitytokens.tokenid = securitygroups.tokenid
			INNER JOIN securityroles ON securitygroups.secroleid = securityroles.secroleid";

	if (!in_array('All', $_POST['Tokens'])){
		$SQL .= " WHERE securitytokens.tokenid IN ('" . implode("','", $_POST['Tokens']) . "')";
	}

	$SQL .= " ORDER BY securitytokens.tokenid, securityroles.secrolename";

	$ErrMsg = __('The user roles could not be retrieved');
	...
	$HTML .= '<meta name="author" content="WebERP " . $Version">
			...
			<body>
				<div class="centre" id="ReportHeader">
					...
					' . __('User Roles by Security Token Report') . '<br />
	...
	while ($MyRow = DB_fetch_array($Result)){
		...
		if ($CurrentToken != $MyRow['tokenid']){
			$HTML .= '<tr>
						<th><h3>' . $MyRow['tokenid'] . ' - ' . $MyRow['tokenname'] . '</h3></th>
					</tr>';

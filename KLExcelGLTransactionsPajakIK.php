...
function submit($FromDate, $ToDate) {
	...
	if ($InputError == 0){
		...
		$SQL = "SELECT accountgroups.groupname AS 'Group',
					...
					chartmasterIK.accountname AS 'AccountName', 
				...
				FROM gltrans, 
					chartmasterIK, 
					accountgroups
				WHERE gltrans.account = chartmasterIK.accountcode
...
// the page is called.
	...
	echo '<p class="page_title_text">
			<img src="' . $RootPath . '/css/' . $Theme . '/images/magnifier.png" title="' . _('GL Transactions for Retail Partner IK (Excel File)') . '" alt="" />' . ' ' . _('GL Transactions for Retail Partner IK (Excel File)') . '
		</p>';

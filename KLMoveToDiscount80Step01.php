...
$Title = __('Move Item To 80% Discount -> Step 01');
...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedMovement) AND $InputError !=1) {
		...
		$Msg = __('Move Item To 80% Discount Step 01 record for') . ' ' . $_POST['Stockid'] . ' ' . __('has been updated');
	} elseif ($InputError !=1) {

		$SQL = "INSERT INTO klmovetodiscount80 
						(stockid,
						startprocessdate,
						discountcategory,
						endprocessdate)
				VALUES ('" . $_POST['Stockid'] . "',
					...
					'80',
					'1000-01-01')";
		$Msg = __('Move Item To 80% Discount Step 01 record for') . ' ' . $_POST['Stockid'] . ' ' . __('has been created');
	}
	if ($InputError !=1) {
		//run the SQL from either of the above possibilites
		DB_Txn_Begin();

		$ErrMsg = __('The insert or update of the Move Item To 80% Discount Step 01 failed because');
		$Result = DB_query($SQL,$ErrMsg);
		prnMsg($Msg , 'success');

		SetRLZeroAtPointOfSales($_POST['Stockid']);
		SetMoveDiscount80Flag(1,$_POST['Stockid']);

		KLSendEmail("MoveToDiscount80Started", "Silent", $_POST['Stockid']);
	...
	$Result = DB_query($SQL);
	$MyRow = DB_fetch_array($Result);
	SetMoveDiscount80Flag(0,$MyRow['stockid']);

	$SQL="DELETE FROM klmovetodiscount80 
		WHERE countermovediscount='". $SelectedMovement."'";
	$ErrMsg = __('The Move Item To 80% Discount Step 01 could not be deleted because');
	$Result = DB_query($SQL,$ErrMsg);

	prnMsg(__('Move Item To 80% Discount Step 01') . ' ' . $SelectedMovement . ' ' . __('has been deleted from the database'),'success');
...
or deletion of the records*/
	...
	$SQL = "SELECT klmovetodiscount80.countermovediscount,
				stockmaster.stockid,
...
echo '<fieldset>
		<legend>' . __('Item to move to 80% discount') . '</legend>';
...
echo OneButtonCenteredForm('submit', __('Start Change To 80% Discount'));
echo '</fieldset>
	</form>';

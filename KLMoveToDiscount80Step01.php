...
$Title = _('Move Item To 80% Discount -> Step 01');
...
if (isset($_POST['submit'])) {
	...
	if (DB_num_rows($Result)==0) {
		...
		$i++;
		prnMsg(_('The Discount Type must be 80 (so far, only 80% discount available for Discount Category)'),'error');
		...
		$i++;
		prnMsg(_('This item is already in 80% DISCOUNT category. No need to move it.'),'error');
		...
		$i++;
		prnMsg(_('This item is already in Move To 80% Discount procedure. No need to do it twice'),'error');
	...
	if ($InputError !=1) {
		...
		$ErrMsg = _('The insert or update of the Move Item To 80% Discount Step 01 failed because');
	...
	prnMsg(_('Move Item To 80% Discount Step 01') . ' ' . $SelectedMovement . ' ' . _('has been deleted from the database'),'success');
...
echo '<fieldset>
		<legend>' . _('Item to move to 80% discount') . '</legend>';
...
echo OneButtonCenteredForm('submit', _('Start Change To 80% Discount'));
echo '</fieldset>
	</form>';

...
$Title = _('General Ledger Account Inquiry for PT.SMH');
...
$SQL = "SELECT chartmasterSMH.accountcode,
			bankaccounts.accountcode AS bankact,
			bankaccounts.currcode,
			chartmasterSMH.accountname
		FROM chartmasterSMH LEFT JOIN bankaccounts
		ON chartmasterSMH.accountcode=bankaccounts.accountcode
...
if (isset($_POST['Show'])) {
	...
	$Result = DB_query("SELECT pandl
				FROM accountgroups
				INNER JOIN chartmasterSMH ON accountgroups.groupname=chartmasterSMH.group_
				WHERE chartmasterSMH.accountcode='" . $SelectedAccount . "'");
	...
	$NameSQL = "SELECT accountname FROM chartmasterSMH WHERE accountcode='" . $SelectedAccount . "'";

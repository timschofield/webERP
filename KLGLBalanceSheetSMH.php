...
if (! isset($_POST['BalancePeriodEnd']) or isset($_POST['SelectADifferentPeriod'])){
	...
	$SQL = "SELECT Sum(CASE WHEN chartdetails.period='" . $_POST['BalancePeriodEnd'] . "' THEN chartdetails.bfwd + chartdetails.actual ELSE 0 END) AS accumprofitbfwd,
		...
		FROM chartmasterSMH INNER JOIN accountgroups
		ON chartmasterSMH.group_ = accountgroups.groupname INNER JOIN chartdetails
		ON chartmasterSMH.accountcode= chartdetails.accountcode
	...
	while ($MyRow=DB_fetch_array($AccountsResult)) {
		...
		if ($MyRow['accountcode'] == $RetainedEarningsAct){
			$SQL = "SELECT SUM(chartdetails.bfwd + chartdetails.actual)
					FROM chartdetails, chartmasterSMH, accountgroups
					WHERE chartdetails.accountcode = chartmasterSMH.accountcode
	...
	echo '<div class="invoice">
		...
		_('Balance Sheet PT. Sungai Mutiara Hitam as at') . ' ' . $BalanceDate .'</b></font>
					</div>
				</th>
			</tr>';

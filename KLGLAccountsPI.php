...
$Title = __('Chart of Accounts Maintenance for POPI');
...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedAccount) AND $InputError !=1) {
		...
		$SQL = "UPDATE chartmasterPI SET accountname='" . $_POST['AccountName'] . "',
						group_='" . $_POST['Group'] . "'
	...
	/*SelectedAccount is null cos no item selected on first time round so must be adding a	record must be submitting new entries */
		...
		$SQL = "INSERT INTO chartmasterPI (accountcode,
						accountname,
						group_)
...
//the link to delete a selected record was clicked instead of the submit button
	$SQL="DELETE FROM chartmasterPI WHERE accountcode= '" . $SelectedAccount ."'";
	$Result = DB_query($SQL);
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedAccount)) {
		...
		$SQL = "SELECT accountcode, accountname, group_ FROM chartmasterPI WHERE accountcode='" . $SelectedAccount ."'";
	...
	echo OneButtonCenteredForm('submit', __('Enter PT. PI GL Account Details'));
...
then none of the above are true and the list of chartmasterPI will be displayed with
...
or deletion of the records*/
	...
	$SQL = "SELECT accountcode,
		...
		FROM chartmasterPI,
			accountgroups
		WHERE chartmasterPI.group_=accountgroups.groupname

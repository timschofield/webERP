...
$Title = _('General Ledger Account Inquiry for PT.ADU');
...
$SQL = "SELECT chartmasterADU.accountcode,
			bankaccounts.accountcode AS bankact,
			bankaccounts.currcode,
			chartmasterADU.accountname
		FROM chartmasterADU LEFT JOIN bankaccounts
		ON chartmasterADU.accountcode=bankaccounts.accountcode
...
if (isset($_POST['Show'])) {
	...
	$Result = DB_query("SELECT pandl
				FROM accountgroups
				INNER JOIN chartmasterADU ON accountgroups.groupname=chartmasterADU.group_
				WHERE chartmasterADU.accountcode='" . $SelectedAccount . "'");
	...
	$NameSQL = "SELECT accountname FROM chartmasterADU WHERE accountcode='" . $SelectedAccount . "'";

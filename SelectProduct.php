...
/*	echo '<p class="page_title_text"><img alt="" src="', $RootPath, '/css/', $Theme,
	...
	if (($MyTopSales = DB_fetch_array($TopSalesResult)) AND 
		...
		echo '<tr>
				<th class="number" style="width:15%">' . __('Top Sales 30 days') . ':</th>
				<td style="width:17%; text-align:right" class="select">' . '# ' . $MyTopSales['topsales30'] . '</td>
			</tr>
			<tr>
				<th class="number" style="width:15%">' . __('Top Sales 60 days') . ':</th>
				<td style="width:17%; text-align:right" class="select">' . '# ' . $MyTopSales['topsales60'] . '</td>
			</tr>
			<tr>
				<th class="number" style="width:15%">' . __('Top Sales 90 days') . ':</th>
	...
	} // KL RICARD END MOD TO SHOW TOP SALES
	echo '<td style="width:15%; vertical-align:top">
	...
	if (($MyRow['mbflag'] == 'B' or ($MyRow['mbflag'] == 'M'))
		...
		// KL RICARD ADD effective from date and limit 10 rows
		echo '<td style = "width:50%" valign = "top"><table>
				<tr><th style = "width:20%">' . __('Supplier') . '</th>
					...
					<th style = "width:5%">' . __('Prefer') . '</th>
					<th style = "width:15%">' . __('From') .
					'</th></tr>';
		// KL RICARD SQL Modified to show max 10 suppliers purch data, not just the latest as standard v4.15.2
	...
	//KL RICARD Not showing standard fix help text as it takes too much real estate and no value
	echo '</td>
		</tr>
	...
	echo '<a href="' . $RootPath . '/PO_SelectOSPurchOrder.php?SelectedStockItem=' . urlencode($StockID) . '">' . __('Search Outstanding Purchase Orders') . '</a><br />';
		...
		$PossibleImageFiles = glob($_SESSION['part_pics_dir'] . '/' . $StockID . '.{png,jpg,jpeg
}', GLOB_BRACE);

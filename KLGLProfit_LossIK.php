...
$Title = _('Profit and Loss for Retail Partner POIK');// Screen identification.
...
//		_('Print Statement of Comprehensive Income') . '</p>';// Page title.
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
				...
				FROM chartmasterIK
					INNER JOIN accountgroups ON chartmasterIK.group_ = accountgroups.groupname
					INNER JOIN chartdetails ON chartmasterIK.accountcode= chartdetails.accountcode
					INNER JOIN glaccountusers ON glaccountusers.accountcode=chartmasterIK.accountcode AND glaccountusers.userid='" .  $_SESSION['UserID'] . "' AND glaccountusers.canview=1
...
//		stripslashes($_SESSION['CompanyRecord']['coyname']) . '<br />' .// Page title, reporting entity.
		_('Retail Partner POIK') . '<br />' .// Page title, reporting statement.
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($_POST['Detail']=='Detailed'){
			...
			if (isset($_POST['ShowZeroBalances']) OR (!isset($_POST['ShowZeroBalances']) AND ($AccountPeriodActual <> 0 OR $AccountPeriodBudget <> 0 OR $AccountPeriodLY <> 0))){
				...
				$ActEnquiryURL = '<a href="' . $RootPath . '/KLGLAccountInquiryIK.php?Period=' . $_POST['ToPeriod'] . '&amp;Account=' . $myrow['accountcode'] . '&amp;Show=Yes">' . $myrow['accountcode'] . '</a>';
				if ($Section ==1){
					 printf('<td>%s</td>
							<td>%s</td>
							<td></td>

...
/*Will only be true if clicked to search for a customer code */
	...
	if ($_POST['Keywords']==''
		...
		if (DB_error_no() !=0) {
			prnMsg(__('The searched customer records requested cannot be retrieved because') . ' - ' . DB_error_msg(),'error');
			if ($Debug==1){
				prnMsg(__('SQL used to retrieve the customer details was') . '<br />' . $SQL,'error');
			}

...
/*First off do the Inventory Comparison file stuff */
	if ($_POST['ReportOrClose']=='ReportAndClose'){
		...
		if (DB_error_no() !=0) {
			$Title = __('Stock Freeze') . ' - ' . __('Problem Report') . '....';
			include('includes/header.php');
			echo '<br />';
			prnMsg( __('The inventory check file could not be retrieved because'). ' - ' . DB_error_msg(),'error');
		...
		while ($MyRow = DB_fetch_array($StockChecks)){
			...
			if (DB_error_no() !=0) {
				$Title = __('Stock Count Comparison') . ' - ' . __('Problem Report') . '....';
				include('includes/header.php');
				echo '<br />';
				prnMsg( __('The inventory counts file could not be retrieved because'). ' - ' . DB_error_msg(). 'error');
				echo '<br /><a href="' .$RootPath .'/index.php">' .  __('Back to the menu') . '</a>';
				if ($Debug==1){
					echo '<br />' .  $SQL;
				}
	...
	while ($CheckItemRow = DB_fetch_array($CheckedItems)){
		...
		if (DB_error_no() !=0) {
	 		$Title = __('Inventory Comparison') . ' - ' . __('Problem Report') . '.... ';
	  		include('includes/header.php');
	   		echo '<br />';
			prnMsg( __('The inventory counts could not be retrieved by the SQL because').' - ' . DB_error_msg(), 'error');
	   		echo '<br /><a href="' .$RootPath .'/index.php">' .  __('Back to the menu'). '</a>';
	   		if ($Debug==1){
	      			echo '<br />' .  $SQL;
	   		}
		...
		if ($CheckItemRow['qoh']!=0 OR DB_num_rows($Counts)>0) {
			...
			if (mb_strlen($CheckItemRow['bin'])>0){
				$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,120,$FontSize,$CheckItemRow['stockid'] . ' - ' . __('Bin') . ':' . $CheckItemRow['bin'], 'left');
			} else {
		...
		if (DB_num_rows($Counts)==0 AND $CheckItemRow['qoh']!=0){
			$LeftOvers = $pdf->addTextWrap(380, $YPos,160, $FontSize, __('No counts entered'), 'left');
			...
			$LeftOvers = $pdf->addTextWrap($Left_Margin, $YPos, 375-$Left_Margin, $FontSize, __('Total for') . ': ' . $CheckItemRow['stockid'], 'right');

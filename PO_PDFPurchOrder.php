...
- Remove logo, some forced fields (Company name, NPWP, telephone, email)
*****************************************************************************************/
...
if (isset($MakePDFThenDisplayIt) or isset($MakePDFThenEmailIt)) {
	...
	h3.total-right { text-align: right; }
	</style></head><body>';

	// KL RICARD MODIFICATION Add warning if viewing only
	if ($ViewingOnly != 0) {
		$HTML .= '<h2>' . __('FOR VIEWING ONLY') . ': ' . __('DO NOT SEND TO SUPPLIER') . '</h2>';
		$HTML .= '<h2>' . __('THIS IS NOT AN ORDER') . '</h2>';
	}
	// KL MODIFICATION END
	// KL RICARD MODIFICATION Add the NPWP, Telf and email
	$CompanyAddress = $_SESSION['TaxAuthorityReferenceName'] . ' ' . NPWP_FOR_PO . '<br />';
	for ($i = 1; $i < 7; $i++) {
		if ($_SESSION['CompanyRecord']['regoffice' . $i] != '') {
			$CompanyAddress .= $_SESSION['CompanyRecord']['regoffice' . $i] . '<br />';
		}
	}
	$CompanyAddress .= $_SESSION['CompanyRecord']['telephone'] . '<br />';
	$CompanyAddress .= $_SESSION['CompanyRecord']['email'] . '<br />';
	// KL RICARD MODIFICATION END
	...
	// KL RICARD MODIFICATION DO NOT SHOW THE LOGO
	// $HTML .= '<img class="logo" src="' . $_SESSION['LogoFile'] . '" /><br />';
	// END KL RICARD MODIFICATION
	...
	$HTML .= '<table class="header-table">
				...
				<tr>
					<td style="vertical-align:top">' . htmlspecialchars(COMPANY_NAME_FOR_PO) . '<br />' . $CompanyAddress . '</td>
	...
	$HTML .= '<table>
		<thead>
			...
			<tr>
				...
				<th>' . __('Quantity') . '</th>
				<th>' . __('Unit') . '</th>
				<th>' . __('Price') . ' ' . htmlspecialchars($POHeader['currcode']) . '</th>
				<th>' . __('Line Total') . ' ' . htmlspecialchars($POHeader['currcode']) . '</th>
			</tr>
		</thead>
		<tbody>';
...
// KL RICARD MODIFICATION END
	...
	$HTML .= '<h3 class="total-right">' . __('Order Total') . ': ' . $DisplayOrderTotal . ' ' . htmlspecialchars($POHeader['currcode']) . '</h3>';

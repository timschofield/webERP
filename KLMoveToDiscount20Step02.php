	...
	if (DB_num_rows($result) != 0){
		...
		while ($myrow = DB_fetch_array($result)) {
			...
			if ((($myrow['qohkantor'] + $myrow['qohotherlocs']) == $myrow['qohtotal'])
				...
				if (($myrow['categoryid']== "DISC20") OR ($myrow['categoryid']== "DISC2A")){
					...
					$NewLabelsPrinted = '<a href="' . $RootPath . '/KLChangeToDiscount20.php?Item=' . $myrow['stockid'] . '&Discount='. $myrow['discountcategory'] . '&Action=Finish">' . ('Printed') . '</a>';
				}else{
					...
					$NewDiscountCategory = '<a href="' . $RootPath . '/KLChangeToDiscount20.php?Item=' . $myrow['stockid'] . '&Discount='. $myrow['discountcategory'] . '&Action=Change">' . $myrow['discountcategory'] . '</a>';

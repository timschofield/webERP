...
if (isset($_FILES['ItemPicture']) and $_FILES['ItemPicture']['name'] != '') {
	...
	//File Type Check
		...
		$UploadTheFile = 'No';
} elseif ($_FILES['ItemPicture']['error'] == 6) {
	//upload temp directory check
...
// KL RICARD: Add length, Width, Height and Units_dimension
	if (filter_number_format($_POST['Length'])<0) {
		$InputError = 1;
		prnMsg(__('The length of the item must be a positive number'),'error');
		$Errors[$i] = 'length';
		$i++;
	}
	if (filter_number_format($_POST['Width'])<0) {
		$InputError = 1;
		prnMsg(__('The width of the item must be a positive number'),'error');
		$Errors[$i] = 'Width';
		$i++;
	}
	if (filter_number_format($_POST['Height'])<0) {
		$InputError = 1;
		prnMsg(__('The height of the item must be a positive number'),'error');
		$Errors[$i] = 'Height';
		$i++;
	}
// END KL RICARD
	...
	/*so its an existing one */
			...
			*/
			// KL RICARD Add kl fields to SQL
			$SQL = "SELECT mbflag,
							controlled,
							serialised,
...
// KL RICARD added the new fields of dimensions
	$SQL = "SELECT stockid,
					description,
					longdescription,
					categoryid,
...
echo '<field>
		<label for = "Volume">' . __('Packaged Volume (m3)') . ':</label>
		...
		<fieldhelp>' . __('Volume of the shipping box containing the item in m3.') . '</fieldhelp>
	</field>';
...
echo '<field>
		...
		<fieldhelp>' . __('Weight of the item + shipping box in KG.') . '</fieldhelp>
	</field>';
...
echo '<field>
		...
		<fieldhelp>' . __('Weight of the item (only) in KG.') . '</fieldhelp>
	</field>';
// KL RICARD PACKAGING
echo '<field>
		<label for = "KLPackaging">' . __('Packaging Set') . ':</label>
...
if (!isset($_POST['KLPackaging']) OR ($_POST['KLPackaging'] == '')) {
	$KLProw['packagingdescription'] = __('-');
}
...
echo '<field>
		<label for = "UnitsDimension">' . __('Units of Dimension') . ':</label>
...
// KL RICARD
echo '<field>
		<label for="KLSyncToOpenCart">' . __('Sync item to OpenCart website?') . ':</label>
		<select name="KLSyncToOpenCart">';

if ($_POST['KLSyncToOpenCart']==0){
	echo '<option selected="selected" value="0">' . __('Do NOT Sync') . '</option>';
} else {
	echo '<option value="0">' . __('Do NOT Sync') . '</option>';
}
...
echo '<field>
		<label for="KLServiceByReplacement">' . __('Item can be serviced ONLY by replacement?') . ':</label>
		<select name="KLServiceByReplacement">';

if ($_POST['KLServiceByReplacement']==0){
	echo '<option selected="selected" value="0">' . __('No, can be normally serviced') . '</option>';
} else {
	echo '<option value="0">' . __('No, can be normally serviced') . '</option>';
}

if ($_POST['KLServiceByReplacement']==1){
	echo '<option selected="selected" value="1">' . __('Yes, only serviceable by replacement with a new one') . '</option>';
} else {
	echo '<option value="1">' . __('Yes, only serviceable by replacement with a new one') . '</option>';
}
echo '</select>
	</field>';
// KL RICARD END

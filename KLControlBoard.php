...
*/
	...
	***************************************************************************************/
	...
	if ($KL_SystemAdmin){
		...
		$NumberOfTestExecuted++;
		BalanceAccountControl("111512000",   50000000,  125000000, $periodnow, $db);
		$NumberOfTestExecuted++;
		...
		$NumberOfTestExecuted++;
		BalanceAccountControl("211021500",  100000000,  300000000, $periodnow, $db);
		$NumberOfTestExecuted++;
...
*/
		...
		$NumberOfTestExecuted++;
		CategoryItemsNotInShop("DISC20", "TOKUB", 2, $RootPath, $db);
		$NumberOfTestExecuted++;
		
		CategoryItemsNotInShop("DISC50", "TOKAR", 2, $RootPath, $db);
		$NumberOfTestExecuted++;
		CategoryItemsNotInShop("DISC50", "TOKUB", 2, $RootPath, $db);
		$NumberOfTestExecuted++;
	...
	if ($KL_SystemAdmin){
		InsuficientStockForShopPackaging('SHPACK', 21, 110, 30, false, $RootPath, $db); // Works for both regular and outlet shop packaging
		$NumberOfTestExecuted++;
	}

	if ($KL_PurchasingTeam){
		InsuficientStockForShopPackaging('SHPACK', 21, 110, 30, true, $RootPath, $db); // Works for both regular and outlet shop packaging
		$NumberOfTestExecuted++;
	}

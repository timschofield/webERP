...
$Title = _('Balance Sheet for Retail partner IK');
...
if (! isset($_POST['BalancePeriodEnd']) or isset($_POST['SelectADifferentPeriod'])){
	...
	$SQL = "SELECT Sum(CASE WHEN chartdetails.period='" . $_POST['BalancePeriodEnd'] . "' THEN chartdetails.bfwd + chartdetails.actual ELSE 0 END) AS accumprofitbfwd,
		...
		FROM chartmasterIK INNER JOIN accountgroups
		ON chartmasterIK.group_ = accountgroups.groupname INNER JOIN chartdetails
		ON chartmasterIK.accountcode= chartdetails.accountcode
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($myrow['accountcode'] == $RetainedEarningsAct){
			$SQL = "SELECT SUM(chartdetails.bfwd + chartdetails.actual)
					FROM chartdetails, chartmasterIK, accountgroups
					WHERE chartdetails.accountcode = chartmasterIK.accountcode
						AND chartmasterIK.group_ = accountgroups.groupname

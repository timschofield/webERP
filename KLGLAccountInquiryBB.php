...
$Title = _('General Ledger Account Inquiry for PT.BB');
...
$SQL = "SELECT chartmasterBB.accountcode,
			bankaccounts.accountcode AS bankact,
			bankaccounts.currcode,
			chartmasterBB.accountname
		FROM chartmasterBB LEFT JOIN bankaccounts
		ON chartmasterBB.accountcode=bankaccounts.accountcode
...
if (isset($_POST['Show'])) {
	...
	$Result = DB_query("SELECT pandl
				FROM accountgroups
				INNER JOIN chartmasterBB ON accountgroups.groupname=chartmasterBB.group_
				WHERE chartmasterBB.accountcode='" . $SelectedAccount . "'");
	...
	$NameSQL = "SELECT accountname FROM chartmasterBB WHERE accountcode='" . $SelectedAccount . "'";

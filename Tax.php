...
if (isset($_POST['TaxAuthority']) and isset($_POST['PrintPDF']) and isset($_POST['NoOfPeriods']) and isset($_POST['ToPeriod'])) {
	...
	if (DB_error_no() != 0) {
		...
		prnMsg(__('The accounts receivable transaction details could not be retrieved because') . ' ' . DB_error_msg(), 'error');
	...
	if (DB_error_no() != 0) {
		$Title = __('Taxation Reporting Error');
		include('includes/header.php');
		echo __('The accounts payable transaction details could not be retrieved because') . ' ' . DB_error_msg();
		echo '<br /><a href="' . $RootPath . '/index.php?">' . __('Back to the menu') . '</a>';
		if ($Debug == 1) {
			echo '<br />' . $SQL;
		}
	...
	if (DB_error_no() != 0) {
		$Title = __('Taxation Reporting Error');
		include('includes/header.php');
		echo __('The petty cash transaction details could not be retrieved because') . ' ' . DB_error_msg();
		echo '<br /><a href="' . $RootPath . '/index.php?">' . __('Back to the menu') . '</a>';
		if ($Debug == 1) {
			echo '<br />' . $SQL;
		}

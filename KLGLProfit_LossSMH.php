...
$Title = _('Profit and Loss for PT. Sungai Mutiara Hitam');// Screen identification.
...
if ((!isset($_POST['FromPeriod'])
	...
	$SQL = "SELECT accountgroups.sectioninaccounts,
				...
				FROM chartmasterSMH
					INNER JOIN accountgroups ON chartmasterSMH.group_ = accountgroups.groupname
					INNER JOIN chartdetails ON chartmasterSMH.accountcode= chartdetails.accountcode
					INNER JOIN glaccountusers ON glaccountusers.accountcode=chartmasterSMH.accountcode AND glaccountusers.userid='" .  $_SESSION['UserID'] . "' AND glaccountusers.canview=1
...
//		stripslashes($_SESSION['CompanyRecord']['coyname']) . '<br />' .// Page title, reporting entity.
		_('PT. Sungai Mutiara Hitam') . '<br />' .// Page title, reporting statement.
...
// RICARD 
		...
		if ($_POST['Detail']=='Detailed'){
			...
			if (isset($_POST['ShowZeroBalances']) OR (!isset($_POST['ShowZeroBalances']) AND ($AccountPeriodActual <> 0 OR $AccountPeriodBudget <> 0 OR $AccountPeriodLY <> 0))){
				...
				$ActEnquiryURL = '<a href="' . $RootPath . '/KLGLAccountInquirySMH.php?Period=' . $_POST['ToPeriod'] . '&amp;Account=' . $MyRow['accountcode'] . '&amp;Show=Yes">' . $MyRow['accountcode'] . '</a>';
				if ($Section ==1){
					 printf('<td>%s</td>
							<td>%s</td>
							<td></td>

...
if (isset($_POST['submit'])) {
	...
	if (DB_num_rows($Result)==0) {
		...
		$i++;
		prnMsg(__('The Discount Type must be 50 (so far, only 50% discount available for Discount Category)'),'error');
	} elseif (GetTotalItemsMovingToDiscount('50') >= $_SESSION['MaxItemsChangingDisc50']) {
		$InputError = 1;
		$Errors[$i] = 'MaxItemsMovingToDiscount';
		$i++;
		prnMsg('Too many items moving to Discount 50% at the same time. Maximum = '. $_SESSION['MaxItemsChangingDisc50'],'error');
	} elseif (ItemInLIst($MyRow['categoryid'], LIST_STOCK_CATEGORIES_DISCOUNT_50)) {
		$InputError = 1;
		$Errors[$i] = 'AlreadyDiscount50';
		$i++;
		prnMsg(__('This item is already in 50% DISCOUNT category. No need to move it.'),'error');
		...
		$i++;
		prnMsg(__('This item is already in Move To 50% Discount procedure. No need to do it twice'),'error');
	...
	if (isset($SelectedMovement) AND $InputError !=1) {
		...
		$Msg = __('Move Item To 50% Discount Step 01 record for') . ' ' . $_POST['Stockid'] . ' ' . __('has been updated');
	} elseif ($InputError !=1) {
		...
		$SQL = "INSERT INTO klmovetodiscount50 
				...
				VALUES ('" . $_POST['Stockid'] . "',
					...
					'50',
					'1000-01-01')";
		$Msg = __('Move Item To 50% Discount Step 01 record for') . ' ' . $_POST['Stockid'] . ' ' . __('has been created');
	}
	if ($InputError !=1) {
		//run the SQL from either of the above possibilites
		DB_Txn_Begin();

		$ErrMsg = __('The insert or update of the Move Item To 50% Discount Step 01 failed because');
	...
	$SQL="DELETE FROM klmovetodiscount50 
		WHERE countermovediscount='". $SelectedMovement."'";
	$ErrMsg = __('The Move Item To 50% Discount Step 01 could not be deleted because');
	$Result = DB_query($SQL,$ErrMsg);

	prnMsg(__('Move Item To 50% Discount Step 01') . ' ' . $SelectedMovement . ' ' . __('has been deleted from the database'),'success');
...
if (isset($SelectedMovement)) {
	echo '<div class="centre"><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . __('Show All Active Movements To 50% Discount') . '</a></div>';
}
...
echo '<fieldset>
		<legend>' . __('Item to move to 50% discount') . '</legend>';
...
echo OneButtonCenteredForm('submit', __('Start Change To 50% Discount'));
echo '</fieldset>
	</form>';

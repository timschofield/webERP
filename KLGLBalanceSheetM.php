...
/* $Id: GLBalanceSheet.php 4837 2012-01-24 23:41:46Z vvs2012 $*/
...
$Title = _('Balance Sheet for Kapal-Laut');
...
if (! isset($_POST['BalancePeriodEnd']) or isset($_POST['SelectADifferentPeriod'])){
	...
	$SQL = "SELECT Sum(CASE WHEN chartdetails.period='" . $_POST['BalancePeriodEnd'] . "' THEN chartdetails.bfwd + chartdetails.actual ELSE 0 END) AS accumprofitbfwd,
		...
		FROM chartmasterM INNER JOIN accountgroups
		ON chartmasterM.group_ = accountgroups.groupname INNER JOIN chartdetails
		ON chartmasterM.accountcode= chartdetails.accountcode
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($myrow['accountcode'] == $RetainedEarningsAct){
			$SQL = "SELECT SUM(chartdetails.bfwd + chartdetails.actual)
					FROM chartdetails, chartmasterM, accountgroups
					WHERE chartdetails.accountcode = chartmasterM.accountcode
						AND chartmasterM.group_ = accountgroups.groupname
						AND accountgroups.sectioninaccounts = 100
						AND chartdetails.period = '" . $_POST['BalancePeriodEnd'] . "'";
			$Earnings = DB_query($SQL,$db);
			...
			$SQL = "SELECT SUM(chartdetails.bfwd + chartdetails.actual)
					...
					WHERE chartdetails.accountcode = chartmasterM.accountcode
						...
						AND chartdetails.period = '" . ($_POST['BalancePeriodEnd']-12) . "'";
			$Earnings = DB_query($SQL,$db);
			$HutangLast=DB_fetch_array($Earnings);
			
			$AccountBalance = -($AktivaNow[0] + $HutangNow[0]);
			$LYAccountBalance = -($AktivaLast[0] + $HutangLast[0]);	
		}
		if ($ActGrp !=''){
	...
	while ($myrow=DB_fetch_array($AccountsResult)) {
		...
		if ($_POST['Detail']=='Detailed'){
			...
			$ActEnquiryURL = '<a href="' . $RootPath . '/KLGLAccountInquiryM.php?Period=' . $_POST['BalancePeriodEnd'] . '&Account=' . $myrow['accountcode'] . '">' . $myrow['accountcode'] . '</a>';
